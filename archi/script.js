...
let controls, clock;
let initialScale = 3; // Stocker l'échelle initiale
...
function loadModel() {
    ...
    const modelUrl = 'https://raw.githubusercontent.com/berru-g/3d-scroll-animate/main/assets/vr_room_light_baked.glb';
...
*/
// Liste des tooltips associés à chaque direction (8 x 45° = 360°)
const tooltips = [
    "Mur de face",
    "Mur avant-droit",
    "Mur droit",
...
function onScroll() {
    ...
    // Rotation de la caméra autour de Y (0° → 360°)
    const angle = scrollPercentage * Math.PI * 2;
    camera.position.x = Math.sin(angle) * 5;  // rayon 5
    camera.position.z = Math.cos(angle) * 5;
    camera.lookAt(0, 0, 0); // Regarde toujours le centre de la pièce

    // --- Détection d’un "cran" de 45° ---
    const step = Math.floor((scrollPercentage * 360) / 45) % 8;
    if (step !== currentTooltipIndex) {
        currentTooltipIndex = step;
        showTooltip(tooltips[step]);
    }
}
